...
async function listarCreditos(
  ...
  opcoes?: { apenasComSaldo?: boolean }
) {
  const apenasComSaldo = opcoes?.apenasComSaldo ?? true
...
async function criarCredito(dados: DadosCriarCredito, companyId: string) {
  ...
  const credito = await clientePrisma.creditoFornecedor.create({
    data: {
      companyId,
...
async function validarCreditoNoPedido(
  ...
  if (creditoFornecedorId) {
    ...
    const disponivel = Number(credito.saldo)
