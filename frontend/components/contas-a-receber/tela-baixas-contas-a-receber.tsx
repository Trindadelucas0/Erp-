...
export function TelaBaixasContasAReceber({ clientes, planos, aoBaixar }: Props) {
  ...
  const [detalhe, setDetalhe] = useState<{ conta: ContaReceberLista; form: FormContaReceber } | null>(
    null
  )
  ...
  async function baixarSelecionados() {
    ...
    for (const l of selecionadas) {
      ...
      if (principal <= 0) {
        setErro(`Informe o valor principal do título ${formatarCodigoContaReceber(l.conta.codigo)}.`)
        return
      }
      if (principal > saldo + 0.009) {
        setErro(
          `Principal do título ${formatarCodigoContaReceber(l.conta.codigo)} passa do saldo (${formatarMoedaBr(saldo)}).`
        )
        return
      }
    }
    ...
    setErro(null)
    try {
      await clienteHttp.post('/contas-a-receber/baixas', {
        pagoEm: pagoEm || null,
  ...
  return (
    <div className="min-w-0 space-y-4">
      <div className="rounded-md border border-border bg-muted/20 px-3 py-2 text-sm text-muted-foreground">
        ...
        <strong className="text-foreground">Multa</strong> entram no recebimento sem reduzir o saldo.
      </div>

      <div className="flex min-w-0 flex-wrap items-end justify-between gap-3">
        <div className="min-w-[10rem] shrink-0">
          <InputPadrao
            rotulo="Data recebimento"
            type="date"
            value={pagoEm}
        ...
        <div className="flex min-w-0 flex-wrap items-end justify-end gap-2 sm:gap-3">
          ...
          {podeEditar && (
            ...
            >
              {baixando ? 'Recebendo…' : `Receber selecionados (${selecionadas.length})`}
            </Button>
          )}
        </div>
      </div>
        ...
        <table className="w-full min-w-[920px] text-left text-sm">
          <thead className="bg-muted/50 text-muted-foreground">
            <tr>
              ...
              <th className={cn(TH, 'max-w-[12rem]')}>Cliente</th>
          ...
          <tbody>
            {carregando ? (
              ...
              linhas.map((linha) => {
                ...
                const baseClass = classeLinhaStatusContaReceber(linha.conta.status, vencido)
                const nomeCliente = linha.conta.pessoa?.nome ?? '—'
                return (
                  <tr
                    key={linha.parcelaId}
                    className={cn(
                  ...
                  >
                    ...
                    <td className={cn(TD, 'font-medium')}>
                      {linha.conta.codigoExibicao ??
                        formatarCodigoContaReceber(linha.conta.codigo)}
                    </td>
                    <td className={TD}>
                      <CelulaVencimentoContaReceber
                    ...
                    <td className={cn(TD, 'max-w-[12rem]')}>
                      <span className="block truncate" title={nomeCliente}>
                        {nomeCliente}
                      </span>
                    </td>
                    ...
                    <td className={TD}>
                      <div className="flex flex-col items-start gap-0.5">
                        <BadgeTipoContaReceber tipo={linha.conta.tipo} className="text-xs" />
                        <BadgeStatusContaReceber status={linha.conta.status} className="text-xs" />
                      </div>
                    </td>
                    <td className={cn(TD, STICKY_RIGHT_PRINCIPAL)}>
                      <input
                        ...
                        disabled={!podeEditar}
                        aria-label={`Principal do título ${formatarCodigoContaReceber(linha.conta.codigo)}`}
                        onChange={(e) =>
                    ...
                    <td className={cn(TD, STICKY_RIGHT_JUROS)}>
                      <input
                        ...
                        disabled={!podeEditar}
                        aria-label={`Juros do título ${formatarCodigoContaReceber(linha.conta.codigo)}`}
                        onChange={(e) =>
                    ...
                    <td className={cn(TD, STICKY_RIGHT_MULTA)}>
                      <input
                        ...
                        disabled={!podeEditar}
                        aria-label={`Multa do título ${formatarCodigoContaReceber(linha.conta.codigo)}`}
                        onChange={(e) =>
      ...
      <Modal
        ...
        titulo={
          detalhe
            ? `Título ${formatarCodigoContaReceber(detalhe.conta.codigo)}`
            : 'Título'
        }
        ...
        cabecalhoExtra={
          detalhe ? (
            <div className="mt-2 flex flex-wrap gap-2">
              <BadgeStatusContaReceber status={detalhe.conta.status} />
              <BadgeTipoContaReceber tipo={detalhe.conta.tipo} />
            </div>
          ) : undefined
        }
        largura="3xl"
      ...
      >
        {detalhe && (
          <FormularioContaReceber
            ...
            planos={planos}
            codigoExibicao={
              detalhe.conta.codigoExibicao ?? formatarCodigoContaReceber(detalhe.conta.codigo)
            }
            somenteLeitura
