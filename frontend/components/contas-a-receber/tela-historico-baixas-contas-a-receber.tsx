...
export function TelaHistoricoBaixasContasAReceber({ recarregarToken = 0 }: Props) {
  ...
  const [detalhe, setDetalhe] = useState<ContaReceberLista | null>(null)
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await clienteHttp.get<{ baixas: HistoricoBaixaLista[] }>(
        '/contas-a-receber/historico-baixas',
        { params }
      )
      setLinhas(data.baixas ?? [])
    } catch (e) {
      setErro(extrairMensagemApi(e, 'Falha ao listar recebimentos.'))
    } finally {
      setCarregando(false)
    }
  ...
  async function abrirDetalhe(contaReceberId: string) {
    setCarregandoDetalhe(true)
    setErro(null)
    try {
      const { data } = await clienteHttp.get<{ conta: ContaReceberLista }>(
        `/contas-a-receber/${contaReceberId}`
      )
  ...
  return (
    <div className="min-w-0 space-y-4">
      <p className="text-sm text-muted-foreground">
        Histórico do que já foi recebido (principal, juros e multa). Clique em uma linha para ver o
        título completo: valor original, o que já recebeu e o que ainda falta.
      </p>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <InputPadrao
          rotulo="Busca (código, doc, cliente)"
          value={q}
            ...
            {carregando ? (
              ...
              linhas.map((linha) => (
                <tr
                  ...
                  onClick={() => void abrirDetalhe(linha.contaReceberId)}
                >
                  <td className="px-3 py-2">{formatarDataBr(linha.pagoEm)}</td>
                  <td className="px-3 py-2 font-medium">
                    {linha.codigoExibicao ?? formatarCodigoContaReceber(linha.codigo)}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <BadgeStatusContaReceber status={linha.statusConta} />
                  </td>
                </tr>
              ))
            )}
      ...
      <Modal
        ...
        titulo={
          detalhe
            ? `Título ${detalhe.codigoExibicao ?? formatarCodigoContaReceber(detalhe.codigo)}`
            : 'Carregando…'
        }
        descricao="Resumo do documento: valor, recebimentos feitos e saldo em aberto."
        largura="3xl"
        rodape={
        ...
        {detalhe && (
          <div className="space-y-4 text-sm">
            <div className="flex flex-wrap gap-2">
              <BadgeStatusContaReceber status={detalhe.status} />
              <BadgeOrigemContaReceber origem={detalhe.origem} />
            </div>
            ...
            <div className="grid gap-2 rounded-md border bg-muted/20 p-3 sm:grid-cols-2">
              ...
              <div>
                <div className="text-muted-foreground">Já recebido (principal)</div>
                <div className="font-medium tabular-nums text-emerald-700">
                  {formatarMoedaBr(detalhe.valorPagoPrincipal ?? 0)}
                </div>
              </div>
              <div>
                <div className="text-muted-foreground">Juros recebidos (baixas)</div>
                <div className="tabular-nums">
                  {formatarMoedaBr(detalhe.totalJurosBaixas ?? 0)}
                </div>
              </div>
              <div>
                <div className="text-muted-foreground">Multas recebidas (baixas)</div>
                <div className="tabular-nums">
            ...
            <div>
              <h3 className="mb-2 font-medium">Recebimentos feitos</h3>
